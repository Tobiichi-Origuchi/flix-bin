...
ORIG_PKG="$(find "$DIST" -type f -name '*.pkg.tar.*' | head -n1)"

mkdir -p "$WORKDIR/repack/orig"
mkdir -p "$WORKDIR/repack/new"

bsdtar -xpf "$ORIG_PKG" -C "$WORKDIR/repack/orig"
...
builddate = $(date +%s)
packager = ${AUR_GIT_NAME} <${AUR_GIT_EMAIL}>
size = $(du -sb "$WORKDIR/repack/orig" | awk '{print $1}')
arch = x86_64
license = custom
EOF

FINAL_PKG="${DIST}/${AUR_PACKAGE_NAME}-${PKGVER}-1-x86_64.pkg.tar.zst"
